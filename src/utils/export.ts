...
import { CHANNEL_LABEL, DIFFICULTY_LABEL, ANNOTATION_SEVERITY_LABEL, CURRENT_EXPORT_VERSION, ANNOTATION_VERSION_CURRENT, CASE_VERSION_CURRENT, REVIEW_VERSION_CURRENT } from '../types';
import { formatDateTime, formatTime } from './uuid';
import { computeReplayHash, loadAnnotations, loadCase, loadReviewItem } from './storage';
...
export function exportReplayJSON(
  ...
  const reviewListItem = loadReviewItem(record.id);
  const payload = {
    ...
    ...(reviewListItem ? { reviewListItem, reviewListVersion: REVIEW_VERSION_CURRENT } : {}),
  };
